...
private ["_name","_cam","_debug","_class","_newClass","_arguments"];
	...
	_monitor setObjectTextureglobal [0,"scripts\DottsKillhouse\LFC\standby.jpg"];
	SLEEP 0.6;
...
private ["_name","_cam","_debug"];
	...
	SLEEP 0.6;
		...
		if (isnil "_cam") 
				then {
					_cam = "camera" camCreate [(getpos _unit select 0), (getpos _unit select 1), (getpos _unit select 2) + 50];	
			...
			_cam setpos [(getpos _unit select 0), (getpos _unit select 1), (getpos _unit select 2) + 50];			
							sleep 0.1;
							};
							};
