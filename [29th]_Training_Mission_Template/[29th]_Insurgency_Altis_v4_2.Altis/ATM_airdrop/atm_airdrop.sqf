...
while {(getPos _target select 2) > 3} do {
	playSound "Vent";
	playSound "Vent2";
	if ((getPos _target select 2) < 0.5) then {
		while {!isTouchingGround _target && speed _target < 5}; _target allowDamage false;
		waitUntil {isTouchingGround _target && speed _target < 3}; _target allowDammage true;
	};
	sleep 5;
};
