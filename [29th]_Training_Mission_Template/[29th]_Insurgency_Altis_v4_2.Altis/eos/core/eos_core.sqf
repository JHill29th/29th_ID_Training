...
//					_newpos=[_mPos, 1500, 3000, 4, 1, 0, 0] call BIS_fnc_findSafePos;
...
//														if (!(vehicle player == _vehicle)) then {{deleteVehicle _x} forEach[_vehicle];};
														_playerIsCrew = 0;//add
														{//add
															if(isPlayer _x) then {//add
																_playerIsCrew = 1;//add
															};//add
														} forEach crew _vehicle;//add
														if (!(vehicle player == _vehicle) && _playerIsCrew == 0) then {{deleteVehicle _x} forEach[_vehicle];};//add
