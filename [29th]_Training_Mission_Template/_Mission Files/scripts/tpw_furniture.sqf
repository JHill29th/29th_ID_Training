...
waitUntil {time > 30;};
systemChat "TPW Furniture is now running.";
 
// VARIABLES
tpw_furniture_version = "1.04"; // Version string
...
tpw_furniture_fnc_populate =
	...
	if (_bld getvariable "tpw_furnished" == 1 && {count (_bld nearEntities ["Man", 7]) == 0}) then
		{
		...
		_spawned = [];
			...
			_item = _item createvehicle [0,0,0];
			_item setposatl _pos;
			_item setdir (_dir - _angle);
			_spawned pushback _item;
			sleep 0.2;	// allow time for object to settle properly, and space out spawning to reduce cpu spike 		
...
while {true} do
	...
	if (tpw_furniture_active && {!isNull player_to_use} && {alive player_to_use} && {player_to_use distance tpw_furniture_lastpos > (tpw_furniture_radius / 2)}) then
		{
		tpw_furniture_lastpos = position player_to_use;
		_blds = player_to_use nearObjects ["House",tpw_furniture_radius];
			{
		...
		for "_i" from 0 to count tpw_furniture_houses - 1 do
			...
			if (_bld distance player_to_use > tpw_furniture_radius && {count (_bld nearEntities ["Man", 7]) == 0}) then
				{
					{
					deletevehicle _x;
					sleep 0.05;
